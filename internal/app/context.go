...
type AppContext struct {
	...
	// Data layer
	State       *state.State
	JSONLReader *storage.JSONLReader
	BeadsDir    string
...
func New(beadsDir string, jsonlReader *storage.JSONLReader) *AppContext {
	return &AppContext{
		BeadsDir:     beadsDir,
...
// RefreshIssues loads issues from JSONL and updates the UI
...
func (ctx *AppContext) RefreshIssues(preserveIssueID string) {
	ctx.App.QueueUpdateDraw(func() {
		...
		issues, err := ctx.JSONLReader.LoadIssues(loadCtx)
		if err != nil {
