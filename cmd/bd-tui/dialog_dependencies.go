...
func (h *DialogHelpers) ShowDependencyDialog() {
	...
	// bd-lite only supports "blocks" type, so no type dropdown needed
	var targetID string
	form.AddInputField("Target Issue ID (blocked by)", "", 20, nil, func(text string) {
		targetID = text
	})
	...
	form.AddButton("Add Dependency", func() {
		...
		log.Printf("BD COMMAND: Adding dependency: bd dep add %s %s", issueID, targetID)
		updatedIssue, err := execBdJSONIssue("dep", "add", issueID, targetID)
		if err != nil {
			...
			h.StatusBar.SetText(fmt.Sprintf("[%s]✓ Now [%s]blocked by[-] [%s]%s[-][-]", formatting.GetSuccessColor(), formatting.GetEmphasisColor(), formatting.GetAccentColor(), targetID))
	...
	if len(issue.Dependencies) > 0 {
		...
		for _, dep := range issue.Dependencies {
			...
			form.AddButton(buttonLabel, func() {
				issueID := issue.ID
				log.Printf("BD COMMAND: Removing dependency: bd dep remove %s %s", issueID, depToRemove.DependsOnID)
				updatedIssue, err := execBdJSONIssue("dep", "remove", issueID, depToRemove.DependsOnID)
				if err != nil {
